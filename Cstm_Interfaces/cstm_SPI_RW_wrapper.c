 ...
 *
 */
void cstm_SPI_RW_Start_wrapper(const uint32_T *SPI_Device, const int_T p_width0,
 ...
 *
 */
void cstm_SPI_RW_Outputs_wrapper(const uint8_T *InputByte,
			uint8_T *OutputByte,
...
unsigned long timeOut = 0;
unsigned long timeOutMax = 1000000;
unsigned char sendBuf[(int)(DATALENGTH+1)];
...
if(REGISTER[0] < 0){
    ...
    for(uint32_t i = 0; i < DATALENGTH[0]; i++){
        if(i < u_width) sendBuf[i] = InputByte[i];
        else sendBuf[i] = 0;
    }
}else{
    ...
    for(uint32_t i = 0; i < DATALENGTH[0]; i++){
        if(i < u_width) sendBuf[(i+1)] = InputByte[i];
        else sendBuf[(i+1)] = 0;
    }
}
