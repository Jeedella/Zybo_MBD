 ...
 *
 */
void cstm_SPI_WRITE_Start_wrapper(const uint32_T *SPI_Device, const int_T p_width0,
 ...
 *
 */
void cstm_SPI_WRITE_Outputs_wrapper(const uint8_T *InputByte,
...
unsigned long timeOut = 0;
unsigned long timeOutMax = 1000000;
unsigned char sendBuf[(u_width+1)];
unsigned char recBuf[(u_width+1)];
...
if(REGISTER[0] < 0){
    ...
    for(uint32_t i = 0; i < u_width; i++){
        sendBuf[i] = InputByte[i];
    }
}else{
    length = u_width + 1;
    sendBuf[0] = REGISTER[0];
    for(uint32_t i = 0; i < u_width; i++){
        sendBuf[(i+1)] = InputByte[i];
    }
}
